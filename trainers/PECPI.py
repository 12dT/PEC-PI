"""
MaPLe with Dual-Stream Fusion + Staged Caption Introduction

Key modification from maple_dual_stream.py:
- Three caption stages are introduced at different time windows
- Stage 1 (Tone): epochs 1-5 (ramp from 0 to full weight)
- Stage 2 (Content): epochs 6-10 (ramp from 0 to full weight)
- Stage 3 (Emotion): epochs 11-15 (ramp from 0 to full weight)

This creates a hierarchical cognitive progression during training.
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # Staged introduction configuration
        self.stage_intro_enable = cfg.TRAINER.MAPLE.CAPTION_LOSS.STAGED.ENABLE
        
        # 灵活配置：可以从cfg读取，或根据RAMP_EPOCHS自动计算
        ramp_epochs = getattr(cfg.TRAINER.MAPLE.CAPTION_LOSS, 'RAMP_EPOCHS', 15)
        
        # 尝试从配置读取自定义设置
        if hasattr(cfg.TRAINER.MAPLE.CAPTION_LOSS.STAGED, 'STAGED_INTRO'):
            config = cfg.TRAINER.MAPLE.CAPTION_LOSS.STAGED.STAGED_INTRO
            self.stage_start_epochs = config.get('START_EPOCHS', [0, 5, 10])
            self.stage_ramp_duration = config.get('DURATION', 5)
        else:
            # 自动计算：根据RAMP_EPOCHS分成三段
            # 例如：RAMP_EPOCHS=15时，分成[0, 5, 10]，每段5个epoch
            segment = max(1, ramp_epochs // 3)
            self.stage_start_epochs = [0, segment, 2 * segment]
            self.stage_ramp_duration = segment
        
        print(f"Staged caption introduction enabled:")
        print(f"  - Stage 1 (Tone): epochs {self.stage_start_epochs[0] + 1}-{self.stage_start_epochs[0] + self.stage_ramp_duration}")
        print(f"  - Stage 2 (Content): epochs {self.stage_start_epochs[1] + 1}-{self.stage_start_epochs[1] + self.stage_ramp_duration}")
        print(f"  - Stage 3 (Emotion): epochs {self.stage_start_epochs[2] + 1}-{self.stage_start_epochs[2] + self.stage_ramp_duration}")
        ...
        if self.prompt_learner.training:
            ...
            if staged_tokens is not None:
                ...
                for idx, tokens in enumerate(staged_tokens):
                    ...
                    # ========== 关键修改：分阶段引入 ==========
                    if hasattr(self, 'current_epoch') and self.stage_intro_enable:
                        if idx < len(self.stage_start_epochs):
                            # 计算当前stage的ramp factor
                            epoch_offset = max(0, self.current_epoch - self.stage_start_epochs[idx])
                            stage_ramp_factor = min(1.0, epoch_offset / float(self.stage_ramp_duration))
                            loss = loss + sw[idx] * stage_ramp_factor * s_loss
                        else:
                            # 所有stage都已完全引入后的处理
                            loss = loss + sw[idx] * s_loss
                    else:
                        # 没有current_epoch或stage_intro_enable时使用原始逻辑
                        if ramp_epochs and hasattr(self, 'current_epoch'):
                            ramp_factor = min(1.0, (self.current_epoch + 1) / float(max(1, ramp_epochs)))
...
class MaPLe(TrainerX):
    def build_model(self):
        ...
        print("Building custom CLIP for Sentiment Analysis (Dual-Stream + Staged Caption Introduction)")
